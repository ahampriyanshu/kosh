...
export async function fetchFlows(now: Date = new Date()): Promise<FlowsSlice> {
  ...
  const researchPrompt =
    ...
    `2) the latest GIFT Nifty level and its change percent; ` +
    `3) the India 10-year government bond yield and its change in basis points; ` +
    `4) upcoming corporate actions (results, dividends, splits, AGMs, bonus) for major NSE stocks in the next ~2 weeks.`;
  const buildStructurePrompt = (research: string) =>
    ...
    `"giftNifty": { value, changePct } or null; "bondYield": { name: "India 10Y", value, changeBps } or null; ` +
