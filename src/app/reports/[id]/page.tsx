...
export async function generateStaticParams() {
  ...
  return manifest.reports.map((r) => ({ id: r.id }));
}
...
export default async function ReportPage({ params }: ReportPageProps) {
  ...
  // Derive title for research (use ticker)
  let title = typeLabel;
  if (envelope.type === 'research') {
    const rc = envelope.content as ResearchContent;
    title = `${rc.ticker} Research`;
  }
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="mt-3 flex items-center gap-4 flex-wrap">
          ...
          <span className="font-mono text-xs text-[var(--color-faint)] truncate max-w-xs" title={envelope.checksum}>
            {envelope.checksum.slice(0, 20)}&hellip;
          </span>
        </div>
      ...
      {envelope.type === 'midsession' && (
        <MidSessionView content={envelope.content as MidSessionContent} />
      )}
      {(envelope.type === 'weekly' || envelope.type === 'monthly') && (
        <RecapView content={envelope.content as RecapContent} />
      )}
      {envelope.type === 'research' && (
        <ResearchView content={envelope.content as ResearchContent} />
      )}
    </div>
  );
}
