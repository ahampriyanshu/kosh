...
1. **Secrets** (repo → Settings → Secrets and variables → Actions):
   ...
   - `RESEND_API_KEY`, `EMAIL_FROM`, `EMAIL_TO` (comma-separated), optional `EMAIL_REPLY_TO`
   - `KITE_API_KEY`, `KITE_ACCESS_TOKEN`, and `PORTFOLIO_KEY` for encrypted portfolio sync. Use `npm run kite:session` with your Kite API secret to refresh the access token.
   - Optional repo **variable** `GOOGLE_MODEL` to override the default Gemini model.
...
2. Add `KITE_API_KEY`, `KITE_API_SECRET`, `KITE_ACCESS_TOKEN`, and `PORTFOLIO_KEY` in GitHub Actions secrets.
...
5. Open `/portfolio` and enter the same value you used for `PORTFOLIO_KEY`.
